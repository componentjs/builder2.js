...
Builder.prototype.resolve = function (branch) {
  ...
  function pushFile(path) {
    ...
    switch (m[0]) {
      case 'js': break
      case 'json': f.transform = 'json'; break
      default: f.transform = 'string'
    }
...
Builder.prototype.append = function* (component, file) {
  ...
  if (transform) js = this.transform[transform](str)

  return this.register(component, file, js, str) + '\n\n'
}
...
Builder.prototype.register = function (component, file, js, str) {
  var self = this
  ...
  var name = file.name
  if (this.dev) {
    js = JSON.stringify(js + '//@ sourceURL=' + str)
    js = js.replace(/\\n/g, '\\n\\\n')
    js = 'require.register("'
      + name
