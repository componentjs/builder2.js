...
Scripts.prototype.lookup = function* (file, target) {
  ...
  if (currentDir || parentDir ) {
    return this.lookupRelative(file, target);
  } else {
  ...
  for (var i = 0; i < localDeps.length; i++) {
    ...
    if (m = re.exec(target)) {
      ...
      if (tail !== '') {
        ...
        relativeLocal = '.' + tail;
        tail = yield* this.lookupRelativeForLocal(branch.locals[dep], relativeLocal);
        debug('resolved relative file for local "' + dep + '/' + tail + '"');
