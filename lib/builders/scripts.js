...
Scripts.prototype.resolve = function* (branch) {
  ...
  obj.path = branch.path || join(this.out, toFolder(branch));
...
Scripts.prototype.lookupRelative = function (file, target) {
  ...
  for (var i = 0; i < files.length; i++) {
    for (var j = 0; j < extensions.length; j++) {
      if (files[i].path === path + extensions[j]) return files[i].name;
    }
  }
...
// concat an array for `[].reduce()`
function concat(a, b) {
  return a.concat(b)
}

// the folder name of a remote component
function toFolder(branch) {
  return branch.name.split('/').join('-') + '-' + branch.ref
}

// the `require` name of a module
